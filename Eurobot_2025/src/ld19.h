...
#define LD19_POINT_PER_PACK 12
#define LD19_FRAME_SIZE 47 // 1(header) + 1(ver_len) + 2(speed) + 2(start_angle) + 12*3(point) + 2(end_angle) + 2(timestamp) + 1(crc8)
...
typedef struct { 
    ...
    LidarPointStructDef point[LD19_POINT_PER_PACK]; 
    uint16_t end_angle; 
    uint16_t timestamp; 
    uint8_t   crc8; 
} LiDARFrameTypeDef;
